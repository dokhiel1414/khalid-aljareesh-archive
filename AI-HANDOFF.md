...
- عدد العناصر الحالي: **≈356 صوتي** (يشمل برنامج «خاتم النبيين» 46 حلقة)، **40 مرئي**، و**≈70 مقروءاً مستقلاً** — وصفحة المقروء `/written` تعرض أيضاً الصوتيات التي تحمل نصاً مقروءاً (انظر القسم 11)، فالمقروء المرئي للزائر أكبر.
...
### قبل الإطلاق الرسمي (2026-05-31، الأحدث)
- **تدوير `AUTH_SECRET` تمّ** عبر `vercel env rm/add AUTH_SECRET production` (CLI). طُبِّق عند إعادة النشر ⇒ أُبطلت جلسات الإدارة (إعادة تسجيل دخول مطلوبة). *(توكن CLI: `env` متاح، `dns` ممنوع.)*
- **حذف روابط alukah.net** من **41 مقالة**: `scripts/clean-alukah.mjs` (يفكّ `<a>...alukah...>` مع إبقاء نص الحاشية `[1]`/التخريج، ويحذف فقرة «رابط الموضوع»). يُشغَّل ببناء مؤقت `node scripts/clean-alukah.mjs run`. **تنبيه**: `/api/search` قد يُرجِع نسخة مخزّنة؛ صفحة `/item/[id]` (force-dynamic) هي المرجع.
- **برنامج «خاتم النبيين»**: 46 حلقة (ح1–ح47 عدا ح36)، PROGRAM، عناوين «خاتم النبيين الحلقة N» (بلا «مع الشيخ…»)، ep=رقم الحلقة. عبر `import-batch.mjs` (manifest `scripts/manifests/khatm.json`).
- **مجموعات التواصل**: قائمة ثابتة `lib/groups.ts` + مكوّن `components/CommunityGroups.tsx`، تظهر في صفحة `/groups` و**الصفحة الرئيسية** (تحت «تصفّح حسب الموضوع») وروابط في التذييل (لا الشريط العلوي).
- **الشريط العلوي**: أُزيلت أيقونة المجموعات، وكُبّر الاسم (`text-[15px] sm:text-xl`). **بطاقة الموضوع `TopicCard`** صُغّرت.
- **تعريب عدد العناصر**: `itemNoun`/`itemCountLabel` في `lib/utils.ts` (٣-١٠→عناصر، >١٠→عنصر، ١→عنصر، ٢→عنصران).
- **خطّ المقالات أثخن**: خط `TradArabicMorph` **ثابت الوزن** (لا `fvar`) فـ`font-weight` بلا أثر ⇒ استُخدم `-webkit-text-stroke` في `.prose` (جسم 0.5px، غامق 1px، عناوين 0.9px) في `globals.css`.
- **فحص ما قبل الإطلاق** (حيّ): كل المسارات 200، الصوت/الفيديو يبثّان، `www` يحوّل (307)، 404 يعمل، الإدارة محميّة. أداة `scripts/delete-items.mjs` متاحة لحذف عنصر بمعرّف Drive.

### الدومين الرسمي + ظهور البحث (2026-05-31)
...
سجل الـcommits الأخير (الأحدث أولاً): `d474f6d` (تنظيف الألوكة + تدوير المفتاح), `7f77029` (تثخين خط المقالات), `2c2dfd0` (خاتم النبيين + الشريط), `c8a1228` (المجموعات بالرئيسية), `f9af816` (صفحة المجموعات), `eb8d0b3` (تحقّق Google), `b1b408e` (الدومين), `f1c5c18` (تغيير الاسم), `3fc1104` (UX/أداء), `b009da7` (SEO).

---

## 12) الأمان (مُعالَج)

- **`AUTH_SECRET` دُوِّر (2026-05-31)**: ولِّد مفتاح جديد وضُبط في Vercel (Production) عبر CLI، والقديم أُزيل. القيمة القديمة كانت مكشوفة في تاريخ Git — لم تعد فعّالة الآن.
- قيمة `AUTH_SECRET` الصريحة أُزيلت من `deploy.ps1` (تُقرأ من `$env:AUTH_SECRET`).
- لوحة الإدارة محميّة بـ `middleware.ts` (تحويل لـ`/admin/login`). بقية المتغيّرات الحسّاسة مشفّرة في Vercel.
- لا بنود أمنية معلّقة حالياً.
